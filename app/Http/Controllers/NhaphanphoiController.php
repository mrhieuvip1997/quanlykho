...
class NhaphanphoiController extends Controller {
	...
	{
		$query = DB::table('nhaphanphoi');
		$search = $req->input('search');
		if ($search) {
			$query->where('npp_ma', 'like','%'.$search.'%');
			$query->orwhere('npp_ten', 'like','%'.$search.'%');
			$query->orwhere('npp_diachi', 'like','%'.$search.'%');
			$query->orwhere('npp_sdt', 'like','%'.$search.'%');
			$query->orwhere('npp_taikhoan', 'like','%'.$search.'%');
			$query->orwhere('npp_email', 'like','%'.$search.'%');
			$query->orwhere('npp_nhanviendaidien', 'like','%'.$search.'%');
		}
		$nhaphanphoi = $query->get();
