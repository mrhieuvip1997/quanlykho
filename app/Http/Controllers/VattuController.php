...
class VattuController extends Controller {
	...
	public function getList(Request $req)
	{
		...
		if ($search) {
			$query->where('vt_ma', 'like','%'.$search.'%');
			$query->orwhere('vt_ten', 'like','%'.$search.'%');
		}
		$vattu = $query->get();
	...
	public function postEdit($id, VattuEditRequest $request)
	{
        ...
        $file = $request->file('image');
	        if($file->getClientOriginalExtension('image')=="jpg||png"){
	          $filename= $file->getClientOriginalName('image');
	          $file->move('anh',$filename);
	        }
	        else {
	          echo "File error, Retry!!";
	        }
     	}
