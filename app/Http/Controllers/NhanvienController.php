...
class NhanvienController extends Controller {
	...
	public function getList(Request $req)
	{
		...
		if ($search) {
			$query->where('nv_ma', 'like','%'.$search.'%');
			$query->orwhere('nv_ten', 'like','%'.$search.'%');
			$query->orwhere('nv_ngaysinh', 'like','%'.$search.'%');
			$query->orwhere('nv_diachi', 'like','%'.$search.'%');
			$query->orwhere('nv_cmnd', 'like','%'.$search.'%');
			$query->orwhere('nv_sdt', 'like','%'.$search.'%');
			$query->orwhere('nv_email', 'like','%'.$search.'%');
		}
		$nhanvien = $query->get();
